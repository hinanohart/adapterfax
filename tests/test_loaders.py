...
def _save(tmp_path: object, name: str, tensors: dict[str, np.ndarray]) -> str:
    path = str(tmp_path / name)  # type: ignore[operator]
    save_file(tensors, path)
    return path


def test_peft_naming(tmp_path: object) -> None:
    p, q, r = 64, 32, 4
    t = {
...
def test_kohya_naming_with_alpha(tmp_path: object) -> None:
    p, q, r = 48, 24, 8
    t = {
...
def test_dora_magnitude(tmp_path: object) -> None:
    p, q, r = 40, 20, 4
    t = {
...
def test_ia3(tmp_path: object) -> None:
...
def test_fail_loud_on_unpaired_weight(tmp_path: object) -> None:
